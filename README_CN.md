...
- 同一窗口内，每份打开的文档独立记住光标与阅读进度，来回切换时自动恢复；首次查看的文档从顶部开始。
- 可拖拽调整的文档侧栏，窄态自动变为“文 / 纲”，支持标题大纲、长文件名省略，只有多文档时才显示关闭按钮。
- 默认预览编辑，随时切换完整 Markdown 源码；切换时保留光标与阅读进度，源码区提供固定行号、可见滚动条和软换行。
- 查找替换支持命中高亮；在预览与源码模式切换匹配时，自动滚动到对应位置并保留查找框焦点。支持大小写、全词、正则、单次替换和全部替换。
...
界面回归检查涵盖查找定位、多文档位置恢复、预览 / 源码切换、新建、关闭、保存和语言切换。
